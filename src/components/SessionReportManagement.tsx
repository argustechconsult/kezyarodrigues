...
import { Appointment, Client, SessionReport } from '../types/types';
...
const SessionReportManagement: React.FC<ReportsProps> = ({
  appointments,
  clients,
  reports,
  setReports,
  ...
  const handleSave = () => {
    ...
    setReports((prev) => {
      const filtered = prev.filter(
        (r) => r.appointmentId !== editingReport.appointmentId,
      );
            ...
            <div className="flex items-center gap-5">
              ...
              <div>
                ...
                <div className="flex items-center gap-3 mt-1">
                  <span className="text-sm font-bold text-teal-600 bg-teal-50 px-3 py-0.5 rounded-full">
                    {selectedClient?.name}
                  </span>
  ...
  return (
    <div className="space-y-6 h-full flex flex-col animate-fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">
            Prontuário e Relatos
          </h2>
          <p className="text-slate-500 text-sm">
            Gerencie o histórico clínico e anotações de cada sessão.
          </p>
        </div>
        <button
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 overflow-hidden min-h-[600px]">
        ...
        {sidebarVisible && (
          <div
            className={`lg:col-span-4 bg-white rounded-3xl shadow-sm border border-slate-100 flex flex-col overflow-hidden transition-all duration-300 ${
              ...
              {filteredClients.length > 0 ? (
                filteredClients.map((client) => (
                  <button
                    ...
                    className={`w-full p-6 flex items-center gap-4 hover:bg-teal-50/30 transition-all text-left group ${
                      selectedClientId === client.id
                        ? 'bg-teal-50/50 border-r-4 border-teal-500'
                        : ''
                    }`}
                  >
                    <div
                      className={`w-12 h-12 rounded-2xl flex items-center justify-center font-bold text-lg shadow-sm transition-all ${
                        selectedClientId === client.id
                    ...
                    <div className="flex-1 overflow-hidden">
                      ...
                      <p className="text-[10px] text-slate-400 truncate uppercase tracking-tighter">
                        Último contato:{' '}
                        {client.lastDate !== '0000-00-00'
                          ? new Date(
                              client.lastDate + 'T12:00:00',
                            ).toLocaleDateString('pt-BR')
                          : 'Sem registro'}
                      </p>
                    </div>
        ...
        <div
          className={`${
            sidebarVisible ? 'lg:col-span-8' : 'lg:col-span-12'
          } bg-white rounded-3xl shadow-sm border border-slate-100 flex flex-col overflow-hidden transition-all duration-300`}
        >
          {selectedClientId ? (
            <>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                      <History className="text-teal-600" size={22} /> Histórico
                      Clínico
                    </h3>
                  ...
                  {clientAppointments.map((app) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full group p-8 rounded-[2rem] border transition-all flex items-center justify-between text-left ${
                          report
                            ? 'bg-white border-slate-200 hover:border-teal-300 hover:shadow-xl hover:shadow-teal-50'
                            : 'bg-slate-50/50 border-slate-100 border-dashed hover:bg-white hover:border-teal-200'
                        }`}
                      >
                        <div className="flex items-center gap-6">
                          <div
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-4 mt-1.5">
                              <span className="text-xs font-black text-slate-400 flex items-center gap-1.5 uppercase tracking-widest">
                                <PenTool size={14} /> {app.type}
                              </span>
                              <span className="w-1.5 h-1.5 rounded-full bg-slate-200"></span>
                              {report ? (
                                <span className="text-xs font-black text-green-600 flex items-center gap-1.5 uppercase tracking-widest">
                                  <Save size={14} /> Prontuário Salvo
                                </span>
                              ) : (
                                <span className="text-xs font-black text-orange-400 flex items-center gap-1.5 italic uppercase tracking-widest">
                                  Aguardando relato
                                </span>
                              )}
                            </div>
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <ChevronRight
                            className="text-slate-200 group-hover:text-teal-500 transition-transform group-hover:translate-x-2"
                            size={28}
                          />
                        </div>
                      </button>
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-12 text-center text-slate-400 bg-slate-50/20">
              ...
              <p className="max-w-md mx-auto text-slate-500 font-medium">
                Selecione um paciente na lista lateral para visualizar e
                gerenciar seu histórico completo de relatos e evoluções
                clínicas.
              </p>
            </div>
          )}
