...
const LoginPage: React.FC<LoginPageProps> = ({ onLogin }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (username === 'admin' && password === 'admin') {
      onLogin();
      // Router redirection is handled in the parent page for cleaner separation,
      // but keeping it here as backup/logic mirror if passed prop onLogin does simpler things.
      // Actually the parent page (src/app/login/page.tsx) handles redirection after login state change.
      // But for button click feedback, we might just rely on parent.
      // However, the original code had navigate('/admin'). wrapper handles logic onLogin.
    } else {
      alert(
      ...
      <div className="w-full max-w-md bg-white rounded-[2.5rem] shadow-2xl p-10 border border-white relative">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-slate-500 mt-2 font-medium">
            Controle clínico Soraia Psicologia
          </p>
        </div>
