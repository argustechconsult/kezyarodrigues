...
const ScheduleManagement: React.FC<ScheduleProps> = ({
  ...
  const handleAddAppointment = (e: React.FormEvent) => {
    ...
    const financialRecord: FinancialRecord = {
      id: `f-${Date.now()}`,
      description: `Agendamento Fono (${app.type}) - ${
        client?.name || 'Cliente'
      }`,
                    ...
                    <div className="flex justify-between items-start mb-6">
                      ...
                      {app.status === 'scheduled' && (
                        <button
                          ...
                          className="text-emerald-600 hover:bg-emerald-50 p-2.5 rounded-xl transition-all shadow-sm border border-emerald-100"
                        >
                          <Check size={20} />
                        </button>
                      )}
                    </div>
                    <h4 className="text-2xl font-black text-slate-900 mb-6 truncate">
                    ...
                    <div className="space-y-4">
                      ...
                      {app.meetLink && app.status === 'scheduled' && (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="flex items-center justify-center gap-3 text-white bg-indigo-600 py-5 rounded-2xl text-sm font-black hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100"
                        >
